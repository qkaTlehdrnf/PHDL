...
def add_item(name_check):
    ...
    item_name=item_name.lower()#Usually name includes capital letter but it saved at database as lower case

    conn = create_connection(database)
    c = conn.cursor()
    try:
        c.execute("SELECT count(*) FROM ph_items WHERE url_name = ?", (item_url_name,))
    except Error as e:
        print(e)
        sys.exit()
